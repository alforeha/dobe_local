...
function renderTopRight(entry: GtdEntry): string | null {
  if (entry.kind === 'system' && entry.isMilestone) return '🏆';
  if (entry.resource) return resolveIcon(entry.resource.type);
  return null;
}
                ...
                return (
                  <GlowRing key={entry.id} active={gtdItemGlows} rounded="lg" className="block">
                    ...
                    >
                      ...
                      <span className="mb-1 text-3xl leading-none">{getMainIcon(entry)}</span>
    ...
    <div className={`min-h-[19rem] rounded-3xl border p-4 shadow-sm ${getTone(entry)}`}>
      <div className="mb-4 flex items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <span className="text-5xl leading-none">{getMainIcon(entry)}</span>
          <div>
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-500 dark:text-gray-300">
              {statKey && (
                <span className="inline-flex items-center gap-1 rounded-full bg-white/70 px-2 py-0.5 dark:bg-gray-900/40">
                  <span>{resolveIcon(statKey)}</span>
                  <span className="capitalize">{statKey}</span>
                </span>
              )}
              {entry.resource && (
                <span className="inline-flex items-center gap-1 rounded-full bg-white/70 px-2 py-0.5 dark:bg-gray-900/40">
                  <span>{resolveIcon(entry.resource.type)}</span>
