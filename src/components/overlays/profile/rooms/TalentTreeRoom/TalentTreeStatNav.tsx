...
export function TalentTreeStatNav({ activeStat, statPoints, onSelect }: TalentTreeStatNavProps) {
  return (
    <div className="shrink-0 flex gap-1 overflow-x-auto border-b border-gray-100 px-4 py-2">
      {STAT_ORDER.map((stat) => (
        <button
          key={stat}
          type="button"
          className={`shrink-0 rounded px-2 py-1 ${
            activeStat === stat ? 'bg-indigo-100 ring-1 ring-indigo-400' : 'hover:bg-gray-100'
          }`}
